...
public class ProjectControllerImpl implements Controller<Project>, MappingController{

	private Model<Project> model;
	private FileDAO<Project> elementDAO;
	private SerializationFileDAOImpl clusterDAO;
	private ClusterController<ProjectCluster> clusterController;
		...
		for (ClusterImpl studentCluster: studentClusterList) {
			...
			for (ClusterImpl projectCluster: projectClusterList) {
				if (Constants.EPSILON > Utils.euclidianDistance(((Project) projectCluster.getClusterElements().get(0)).getPoint(), averageSkills)) {
					...
					if (!flag)
						mappingResult.put(studentCluster, firstProjectClusterList.get(0));
